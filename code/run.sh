    ...
    for BLOCK_SIZE_Y in ${Y_VALUES[@]}; do
        for ((N=1; N<=REPETITIONS; N++)); do
         #echo -n "$N;" | tee -a ./$OUTPUT_FILE
          ...
          CUDA_VISIBLE_DEVICES=$GPU_ID $EXE ../data/tessina_header.txt ../data/tessina_dem.txt ../data/tessina_source.txt $OUT $SIMULATION_STEPS $BLOCK_SIZE_X $BLOCK_SIZE_Y | tee -a ./$OUTPUT_FILE
        done
      done
    done
}
