                               ...
                               const real_t* const __restrict__ Sz, const real_t* const __restrict__ Sh, real_t* const __restrict__ Sh_next)
{
    ...
    for(uint8_t n = 0; n < ADJACENT_CELLS; ++n)
    ...
    for (uint8_t n = 1; n < NEIGBORHOOD_SIZE; n++)
        if (!eliminated_cells[n])
        {
            ...
            real_t f_out_shared = BUF_GET(Sf_shared, blockDim.y, blockDim.x, n - 1, ti, tj);
...
class SciddicaTCudaCfAMe: public SciddicaTCuda<SciddicaTCudaCfAMe>
{
    friend class SciddicaTCuda<SciddicaTCudaCfAMe>;

public:
    SciddicaTCudaCfAMe(char **argv): SciddicaTCuda(argv) {}
...
protected:
    void init_extras()
    {
        ...
        if (!util::checkShmemSizePerBlock(shmem_size))
            exit(EXIT_FAILURE);
    }
...
{
    SciddicaTCudaCfAMe sciddicaT(argv);
